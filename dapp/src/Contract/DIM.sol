...
contract DecentralizedIM {
    ...
    mapping(address => CredentialRequest[]) public credentialRequests; // issuer => requests

    address public admin;

    event IdentityRegistered(address user, string metadata);
    event CredentialRequested(address indexed requester, address indexed issuer, string requestData);
    ...
    event CredentialRequestReviewed(address indexed issuer, uint256 index, bool approved);
    ...
    function requestCredential(address issuer, string memory requestData) external onlyRegistered {
        require(approvedIssuers[issuer], "Issuer not approved");

        credentialRequests[issuer].push(CredentialRequest({
    ...
    function approveRequest(uint256 index, string memory credentialHash) external onlyIssuer {
        ...
        credentials[req.requester].push(Credential({
        ...
        emit CredentialRequestReviewed(msg.sender, index, true);
    }
    ...
    function rejectRequest(uint256 index) external onlyIssuer {
        ...
        req.isReviewed = true;
        emit CredentialRequestReviewed(msg.sender, index, false);
    }
    ...
    function getCredentialRequests(address issuer) external view returns (CredentialRequest[] memory) {
        return credentialRequests[issuer];
    }
